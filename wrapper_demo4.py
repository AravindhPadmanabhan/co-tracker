...
class WrapperDemo:
    def __init__(self, model, grid_size, device):
        ...
        self.device = device
        self.queries = torch.zeros(1, grid_size*grid_size, 3).to(device)
    ...
    def process_step(self, window_frames, is_first_step, *args):
        ...
        if use_onnx:
            return onnx_inference("./cotracker_wrapper.onnx", video_chunk, self.queries, *args)
...
if __name__ == "__main__":
    ...
    online_ind = torch.tensor([0]).to(DEFAULT_DEVICE)
    online_track_feat = torch.tensor(torch.nan).expand(model.model.corr_levels,1,1,grid_size*grid_size,model.model.latent_dim).to(DEFAULT_DEVICE)
    online_track_support = torch.tensor(torch.nan).expand(model.model.corr_levels,1,(2*model.model.corr_radius + 1)**2,grid_size*grid_size,model.model.latent_dim).to(DEFAULT_DEVICE)
    online_coords_predicted = torch.tensor([0.0]).expand(1,model.step,grid_size*grid_size,2).to(DEFAULT_DEVICE)
    online_vis_predicted = torch.tensor([0.0]).expand(1,model.step,grid_size*grid_size).to(DEFAULT_DEVICE)
    online_conf_predicted = torch.tensor([0.0]).expand(1,model.step,grid_size*grid_size).to(DEFAULT_DEVICE)
    ...
    for i, frame in enumerate(
        ...
        if i % model.step == 0 and i != 0:
            results = wrapper_demo.process_step(
                window_frames,
                is_first_step,
                online_ind,
                online_track_feat,
            ...
            results[0] = results[0] * results[0].new_tensor([(frame.shape[1] - 1) / (model.interp_shape[1] - 1), (frame.shape[0] - 1) / (model.interp_shape[0] - 1),])
    ...
    video = torch.tensor(np.stack(window_frames[:48]), device=DEFAULT_DEVICE).permute(
        0, 3, 1, 2
    )[None]
