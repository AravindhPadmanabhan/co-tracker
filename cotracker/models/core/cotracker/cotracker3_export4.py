...
from cotracker.models.core.cotracker.cotracker3_base_export import CoTrackerThreeBaseExport, posenc
...
class CoTrackerThreeExport(CoTrackerThreeBaseExport):
    def __init__(self, **args):
        ...
        self.epsilon = torch.tensor(1e-12, device=DEFAULT_DEVICE)
        self.posenc_scales = torch.tensor([2**i for i in range(0, 10)], dtype=torch.float32, device=DEFAULT_DEVICE)
        # self.S = torch.tensor([self.window_len]).to(DEFAULT_DEVICE)
        self.S = self.window_len
    ...
    def forward_window(
        ...
        for it in range(iters):
            ...
            for i in range(self.corr_levels):
                corr_feat = self.get_correlation_feats(
                    fmaps_pyramid[i], coords_init / 2**i, corr_level=i
                )
                track_feat_support = (
            ...
            rel_coords_forward = rel_coords_forward / self.scale
            rel_coords_backward = rel_coords_backward / self.scale
    ...
    def forward(
        ...
        # S = torch.tensor([self.window_len]).to(device)
        # self.S = 16
        step = self.S // 2  # How much the sliding window moves at every step
        ...
        # if torch.isnan(online_coords_predicted).any():
        ...
        # Determine the presence of NaNs
        # nan_mask = torch.isnan(online_coords_predicted).any(dim=(-1, -2), keepdim=True)  # Mask where NaNs exist

        # Pad logic applied always
        # pad = torch.clamp(torch.min(step, T - step), min=0).to(torch.long)  # Calculate pad size safely
        # pad = step # assume online_* is sent with T=8 and window sent is always 16
        coords_predicted = F.pad(
            online_coords_predicted, (0, 0, 0, 0, 0, 8), "constant"
        )
        vis_predicted = F.pad(
            online_vis_predicted, (0, 0, 0, 8), "constant"
        )
        conf_predicted = F.pad(
            online_conf_predicted, (0, 0, 0, 8), "constant"
        )
        ...
        ind_zero_mask = online_ind == 0
        left = torch.where(ind_zero_mask, online_ind, online_ind+step)
        right = online_ind + self.S
        ...
        for i in range(self.corr_levels):
            ...
            # if torch.isnan(online_track_feat).any():
            ...
            #     )
            nan_mask = torch.isnan(online_track_feat).any()
            online_track_feat[i] = torch.where(nan_mask, torch.zeros_like(track_feat, device=device), online_track_feat[i])
            online_track_support[i] = torch.where(nan_mask, torch.zeros_like(track_feat_support, device=device), online_track_feat[i])

            online_track_feat[i] = online_track_feat[i] + track_feat * sample_mask
            online_track_support[i] = online_track_support[i] + track_feat_support * sample_mask
        ...
        vis_init_zero = torch.zeros((B, self.S, N, 1), device=device).float()
        conf_init_zero = torch.zeros((B, self.S, N, 1), device=device).float()
        coords_init_zero = queried_coords.reshape(B, 1, N, 2).expand(B, self.S, N, 2).float()

        # num_windows = (T - self.S + step - 1) // step + 1
        ...
        coords_init = torch.where(
            copy_over.expand_as(coords_init_zero), coords_prev, coords_init_zero
        )
        vis_init = torch.where(
            copy_over.expand_as(vis_init_zero), vis_prev, vis_init_zero
        )
        conf_init = torch.where(
            copy_over.expand_as(conf_init_zero), conf_prev, conf_init_zero
        )

        coords_init = torch.where(ind_zero_mask, coords_init_zero, coords_init)
        vis_init = torch.where(ind_zero_mask, vis_init_zero, vis_init)
        conf_init = torch.where(ind_zero_mask, conf_init_zero, conf_init)

        attention_mask = (queried_frames < ind + self.S).reshape(B, 1, N)  # B self.S N
        ...
        coords, viss, confs = self.forward_window(
            ...
            vis=vis_init,
            conf=conf_init,
            attention_mask=attention_mask.repeat(1, self.S, 1),
            iters=iters,
            add_space_attn=add_space_attn,
        )
        S_trimmed = (T)  # accounts for last window duration
        coords_predicted[:, ind : ind + self.S] = coords[-1][:, :S_trimmed]
        vis_predicted[:, ind : ind + self.S] = viss[-1][:, :S_trimmed]
        conf_predicted[:, ind : ind + self.S] = confs[-1][:, :S_trimmed]
                
        online_ind = online_ind + step
        online_coords_predicted = coords_predicted
        online_vis_predicted = vis_predicted
        online_conf_predicted = conf_predicted
