...
from cotracker.models.core.model_utils_export import sample_features5d, bilinear_sampler
...
class CoTrackerThreeBaseExport(nn.Module):
    def __init__(
        self,
        window_len=8,
        stride=4,
        ...
        super(CoTrackerThreeBaseExport, self).__init__()
        ...
        self.corr_feat_scales = torch.stack([torch.tensor([2.0/127, 2.0/95, 2.0]),
                                            torch.tensor([2.0/63, 2.0/47, 2.0]),
                                            torch.tensor([2.0/31, 2.0/23, 2.0]),
                                            torch.tensor([2.0/15, 2.0/11, 2.0])], dim=0).to(DEFAULT_DEVICE)
        self.track_feat_scales = torch.stack([torch.tensor([2.0/127, 2.0/95, 2.0/15]),
                                            torch.tensor([2.0/63, 2.0/47, 2.0/15]),
                                            torch.tensor([2.0/31, 2.0/23, 2.0/15]),
                                            torch.tensor([2.0/15, 2.0/11, 2.0/15])], dim=0).to(DEFAULT_DEVICE)
    ...
    def get_track_feat(self, fmaps, queried_frames, queried_coords, support_radius=0, corr_level=1):
        ...
        support_track_feats = sample_features5d(fmaps, support_points, scale=self.track_feat_scales[corr_level])
        return (
            support_track_feats[:, None, support_track_feats.shape[1] // 2],
            support_track_feats,
        )

    def get_correlation_feats(self, fmaps, queried_coords, corr_level):
        ...
        correlation_feat = bilinear_sampler(
            fmaps.reshape(B * T, D, 1, H_, W_), support_points, scale=self.corr_feat_scales[corr_level]
        )
